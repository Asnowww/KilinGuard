...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    match rest[0].as_str() {
        ...
        "plugins" | "plugin" | "marketplace" => {
            ...
            let target = if matches!(action.as_deref(), Some("rollback")) {
                tail.get(1..).map(|parts| parts.join(" "))
            } else {
                tail.get(1).cloned()
            };
            let max_tail_len = if matches!(action.as_deref(), Some("rollback")) {
                3
            } else {
                2
            };
            if tail.len() > max_tail_len {
                ...
                return Err(format!(
                    "unexpected extra arguments after `claw {} {}`: {}\nUsage: claw plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|versions <id>|rollback <id> <version>|help]",
                    rest[0],
                    tail[..2].join(" "),
                    tail[2..].join(" ")
                ));
...
fn check_sandbox_health(status: &runtime::SandboxStatus) -> DiagnosticCheck {
    ...
    let mut details = vec![
        ...
        format!("OverlayFS live   {}", status.overlayfs_active),
        format!("Timeout ms       {}", status.default_timeout_ms),
        format!("Memory limit     {}", status.memory_limit_bytes),
        format!("Output limit     {}", status.output_limit_bytes),
    ];
    ...
    .with_data(Map::from_iter([
        ...
        (
            "overlayfs_supported".to_string(),
            json!(status.overlayfs_supported),
        ),
        (
            "overlayfs_active".to_string(),
            json!(status.overlayfs_active),
        ),
        (
            "overlayfs_merge_required".to_string(),
            json!(status.overlayfs_merge_required),
        ),
        (
            "default_timeout_ms".to_string(),
            json!(status.default_timeout_ms),
        ),
        (
            "memory_limit_bytes".to_string(),
            json!(status.memory_limit_bytes),
        ),
        (
            "output_limit_bytes".to_string(),
            json!(status.output_limit_bytes),
        ),
...
#[derive(Debug, Deserialize)]
struct ListMcpPromptsRequest {
    server: Option<String>,
}

#[derive(Debug, Deserialize)]
struct GetMcpPromptRequest {
    server: String,
    name: String,
    arguments: Option<serde_json::Value>,
}
...
fn merged_runtime_mcp_servers(
    ...
    for (server_name, manifest) in plugin_servers {
        ...
        if external_plugin && manifest.transport == PluginMcpTransport::Stdio {
            servers.insert(
                server_name.clone(),
                runtime::ScopedMcpServerConfig {
                    required: false,
                    scope: ConfigSource::Project,
                    config: runtime::McpServerConfig::Sdk(runtime::McpSdkServerConfig {
                        name: format!(
                            "blocked-external-stdio:{server_name}:requires-kylin-linux-sandbox"
                        ),
                    }),
                },
            );
        ...
        let config = match manifest.transport {
            PluginMcpTransport::Stdio => {
                runtime::McpServerConfig::Stdio(runtime::McpStdioServerConfig {
                    command: manifest.command.ok_or_else(|| {
                        format!("plugin MCP server `{server_name}` is missing stdio command")
                    })?,
                    args: manifest.args,
                    env: {
                        let mut env = manifest.env;
                        if let Some(protocol_version) = manifest.protocol_version {
                            env.insert("CLAWD_MCP_PROTOCOL_VERSION".to_string(), protocol_version);
                        }
                        env.insert(
            ...
            PluginMcpTransport::Sse => {
                runtime::McpServerConfig::Sse(runtime::McpRemoteServerConfig {
                    url: manifest.url.ok_or_else(|| {
                        format!("plugin MCP server `{server_name}` is missing SSE url")
                    })?,
...
fn serde_to_runtime_json(value: Value) -> RuntimeJsonValue {
    match value {
        Value::Null => RuntimeJsonValue::Null,
        Value::Bool(value) => RuntimeJsonValue::Bool(value),
        Value::Number(value) => RuntimeJsonValue::Number(value.as_i64().unwrap_or_default()),
        Value::String(value) => RuntimeJsonValue::String(value),
        Value::Array(values) => {
            RuntimeJsonValue::Array(values.into_iter().map(serde_to_runtime_json).collect())
        }
        Value::Object(entries) => RuntimeJsonValue::Object(
            entries
                .into_iter()
                .map(|(key, value)| (key, serde_to_runtime_json(value)))
                .collect(),
        ),
    }
}
...
fn mcp_wrapper_tool_definitions() -> Vec<RuntimeToolDefinition> {
    vec![
        ...
        RuntimeToolDefinition {
            ...
            description: Some("Get a specific MCP prompt from a configured server.".to_string()),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "server": { "type": "string" },
            ...
            CliOutputFormat::Json => {
                ...
                if matches!(action_str, "help" | "-h" | "--help") {
                    ...
                    let obj = json!({
                        ...
                        "unexpected": null,
                        "usage": {
                            "direct_cli": "claw plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|versions <id>|rollback <id> <version>|help]",
                            "slash_command": "/plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|versions <id>|rollback <id> <version>|help]",
                        },
                        "cwd": cwd_str,
                    });
                    println!("{}", serde_json::to_string_pretty(&obj)?);
...
fn status_json_value(
    ...
    json!({
        ...
        "sandbox": {
            ...
            "overlayfs_supported": context.sandbox_status.overlayfs_supported,
            "overlayfs_active": context.sandbox_status.overlayfs_active,
            "overlayfs_merge_required": context.sandbox_status.overlayfs_merge_required,
            "allowed_mounts": context.sandbox_status.allowed_mounts,
            "default_timeout_ms": context.sandbox_status.default_timeout_ms,
            "memory_limit_bytes": context.sandbox_status.memory_limit_bytes,
            "output_limit_bytes": context.sandbox_status.output_limit_bytes,
...
fn format_sandbox_report(status: &runtime::SandboxStatus) -> String {
  ...
  Fallback reason   {}",
        ...
        status.overlayfs_active,
        status.overlayfs_merge_required,
...
fn sandbox_json_value(status: &runtime::SandboxStatus) -> serde_json::Value {
    ...
    json!({
        ...
        "overlayfs_supported": status.overlayfs_supported,
        "overlayfs_active": status.overlayfs_active,
        "overlayfs_merge_required": status.overlayfs_merge_required,
        ...
        "output_limit_bytes": status.output_limit_bytes,
        "invocation": status.invocation.as_ref().map(|invocation| json!({
            "id": invocation.id.as_str(),
            "root_dir": invocation.root_dir.display().to_string(),
            "home_dir": invocation.home_dir.display().to_string(),
            "temp_dir": invocation.temp_dir.display().to_string(),
            "overlay_plan": invocation.overlay_plan.as_ref().map(|plan| json!({
                "lower_dir": plan.lower_dir.display().to_string(),
                "upper_dir": plan.upper_dir.display().to_string(),
                "work_dir": plan.work_dir.display().to_string(),
                "merged_dir": plan.merged_dir.display().to_string(),
                "marker_path": plan.marker_path.display().to_string(),
            })),
        })),
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Output           namespace, network, OverlayFS, resource limits, and fallback details
  Formats          text (default), json
  ...
  Usage            claw plugins [list|show <name>|install <path>|enable <name>|disable <name>|uninstall <name>|update <name>|versions <name>|rollback <name> <version>]
    ...
    fn removed_login_and_logout_subcommands_error_helpfully() {
        ...
        assert_eq!(
            parse_args(&[
                "plugins".to_string(),
                "versions".to_string(),
                "demo@external".to_string(),
            ])
            .expect("plugins versions <target> should parse"),
            CliAction::Plugins {
                action: Some("versions".to_string()),
        ...
        assert_eq!(
            ...
            .expect("plugins rollback <target> <version> should parse"),
            CliAction::Plugins {
                action: Some("rollback".to_string()),
                target: Some("demo@external 1.0.0".to_string()),
                output_format: CliOutputFormat::Text,
            }
        );
        assert_eq!(
...
"#,
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains(
            "/plugin [list|install <path>|enable <name>|disable <name>|uninstall <id>|update <id>|versions <id>|rollback <id> <version>]"
        ));
...
UU conflicted.rs",
    ...
    fn build_runtime_plugin_state_discovers_mcp_tools_and_surfaces_pending_servers() {
        ...
        let prompt = executor
            .execute(
                "GetMcpPromptTool",
                r#"{"server":"alpha","name":"triage","arguments":{"service":"api"}}"#,
            )
            .expect("prompt should get");
    ...
    #[test]
    fn external_plugin_stdio_mcp_is_fail_closed_until_linux_sandbox_is_unified() {
        let config_home = temp_dir();
        let workspace = temp_dir();
        let source_root = temp_dir();
        fs::create_dir_all(&config_home).expect("config home");
        ...
        let runtime_config = loader.load().expect("runtime config should load");
        let state = build_runtime_plugin_state_with_loader(&workspace, &loader, &runtime_config)
            .expect("plugin MCP runtime state should load");

        let qualified_server = "plugin-mcp-runtime@external::embedded";
        let qualified_tool = runtime::mcp_tool_name(qualified_server, "echo");
        let error = state
            .tool_registry
            .normalize_allowed_tools(&[qualified_tool.clone()])
            .expect_err("external stdio MCP tool must not enter the tool surface");
        assert!(error.contains("unsupported tool"));
        let mcp_state = state.mcp_state.as_ref().expect("degraded MCP state");
        let report = mcp_state
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .degraded_report()
            .expect("external stdio refusal should be structured");
        assert!(report.failed_servers.iter().any(|server| {
            server.server_name == qualified_server
                && server.error.message.contains("blocked-external-stdio")
        }));
...
fn write_mcp_server_fixture(script_path: &Path) {
    let script = [
            ...
            "                    {",
            "                        'name': 'triage',",
            "                        'description': 'Build an incident triage prompt',",
            "                        'arguments': [",
            ...
            "            'id': request['id'],",
            "            'result': {",
            "                'description': f\"Triage {args.get('service', '')}\",",
            "                'messages': [",
            "                    {",
            "                        'role': 'user',",
            "                        'content': {'type': 'text', 'text': f\"Inspect {args.get('service', '')}\"}",
            "                    }",
            "                ]",
            "            }",
            "        })",
    ...
    #[cfg(unix)]
    {
        ...
        let mut permissions = fs::metadata(script_path)
            ...
            .permissions();
        permissions.set_mode(0o755);
        fs::set_permissions(script_path, permissions).expect("mcp fixture chmod");
    }
}
...
mod sandbox_report_tests {
    ...
    fn sandbox_report_renders_expected_fields() {
        ...
        assert!(report.contains("OverlayFS active"));
