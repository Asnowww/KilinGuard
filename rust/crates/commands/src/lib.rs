...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        argument_hint: Some(
            "[list|install <path>|enable <name>|disable <name>|uninstall <id>|update <id>|versions <id>|rollback <id> <version>]",
        ),
        resume_supported: false,
    },
    SlashCommandSpec {
...
fn parse_plugin_command(args: &[&str]) -> Result<SlashCommand, SlashCommandParseError> {
    match args {
        ...
        ["versions"] | ["list-versions"] => Err(usage_error("plugin versions", "<id>")),
        ["versions", target] | ["list-versions", target] => Ok(SlashCommand::Plugins {
            action: Some("versions".to_string()),
            target: Some((*target).to_string()),
        }),
        ["versions", ..] | ["list-versions", ..] => Err(command_error(
            "Unexpected arguments for /plugin versions.",
            "plugin",
            "/plugin versions <id>",
        )),
        ["rollback"] => Err(usage_error("plugin rollback", "<id> <version>")),
        ["rollback", plugin_id, version] => Ok(SlashCommand::Plugins {
            action: Some("rollback".to_string()),
            target: Some(format!("{plugin_id} {version}")),
        }),
        ["rollback", ..] => Err(command_error(
            "Unexpected arguments for /plugin rollback.",
            "plugin",
            "/plugin rollback <id> <version>",
        )),
        [action, ..] => Err(command_error(
            &format!(
                "Unknown /plugin action '{action}'. Use list, install <path>, enable <name>, disable <name>, uninstall <id>, update <id>, versions <id>, or rollback <id> <version>."
            ),
            "plugin",
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/plugins versions demo@external"),
            Ok(Some(SlashCommand::Plugins {
                action: Some("versions".to_string()),
        ...
        assert_eq!(
            SlashCommand::parse("/plugins rollback demo@external 1.0.0"),
            Ok(Some(SlashCommand::Plugins {
                action: Some("rollback".to_string()),
...
"#,
    ...
    #[test]
    fn lists_plugin_versions_and_rolls_back() {
        let config_home = temp_dir("versions-home");
        let source_root = temp_dir("versions-source");
        write_external_plugin(&source_root, "demo", "1.0.0");
        ...
        let versions =
            handle_plugins_slash_command(Some("versions"), Some("demo@external"), &mut manager)
                .expect("versions command should succeed");
        ...
        let rollback = handle_plugins_slash_command(
            Some("rollback"),
            Some("demo@external 1.0.0"),
            &mut manager,
        )
        .expect("rollback command should succeed");
        assert!(rollback.reload_runtime);
