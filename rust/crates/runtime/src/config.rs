...
pub struct RuntimePermissionRuleConfig {
    ...
    role_max: BTreeMap<String, String>,
    templates: BTreeMap<String, RuntimePermissionTemplateConfig>,
    default_lease_seconds: Option<u64>,
    session_timeout_seconds: Option<u64>,
}

#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct RuntimePermissionTemplateConfig {
    allow: Vec<String>,
    deny: Vec<String>,
    ask: Vec<String>,
    max_mode: Option<String>,
...
impl RuntimePermissionRuleConfig {
    ...
    pub fn new(
        ...
        Self {
            ...
            denied_tools,
            max_mode: None,
            role_max: BTreeMap::new(),
            templates: BTreeMap::new(),
            default_lease_seconds: None,
    ...
    #[must_use]
    pub fn max_mode(&self) -> Option<&str> {
        self.max_mode.as_deref()
    }

    #[must_use]
    pub fn role_max(&self) -> &BTreeMap<String, String> {
        &self.role_max
    }

    #[must_use]
    pub fn templates(&self) -> &BTreeMap<String, RuntimePermissionTemplateConfig> {
        &self.templates
    }

    #[must_use]
    pub fn default_lease_seconds(&self) -> Option<u64> {
        self.default_lease_seconds
    }

    #[must_use]
    pub fn session_timeout_seconds(&self) -> Option<u64> {
        self.session_timeout_seconds
    }
}

impl RuntimePermissionTemplateConfig {
    #[must_use]
    pub fn new(
        allow: Vec<String>,
        deny: Vec<String>,
...
fn parse_optional_permission_rules(
    ...
    Ok(RuntimePermissionRuleConfig {
        ...
        .unwrap_or_default(),
        max_mode: optional_string(permissions, "maxMode", "merged settings.permissions")?
            .map(str::to_string),
        role_max: optional_string_map(permissions, "roleMax", "merged settings.permissions")?
            .unwrap_or_default(),
        templates: parse_optional_permission_templates(permissions)?,
        default_lease_seconds: optional_u64(
            permissions,
            "defaultLeaseSeconds",
            "merged settings.permissions",
        )?,
        session_timeout_seconds: optional_u64(
            permissions,
...
fn parse_optional_permission_templates(
    permissions: &BTreeMap<String, JsonValue>,
) -> Result<BTreeMap<String, RuntimePermissionTemplateConfig>, ConfigError> {
    let Some(templates_value) = permissions.get("templates") else {
        return Ok(BTreeMap::new());
    };
    let templates = expect_object(templates_value, "merged settings.permissions.templates")?;
    templates
        .iter()
        .map(|(name, value)| {
            let context = format!("merged settings.permissions.templates.{name}");
            let template = expect_object(value, &context)?;
            Ok((
                name.clone(),
                RuntimePermissionTemplateConfig::new(
                    optional_string_array(template, "allow", &context)?.unwrap_or_default(),
                    optional_string_array(template, "deny", &context)?.unwrap_or_default(),
                    optional_string_array(template, "ask", &context)?.unwrap_or_default(),
                    optional_string(template, "maxMode", &context)?.map(str::to_string),
                ),
            ))
        })
        .collect()
...
fn parse_optional_sandbox_config(root: &JsonValue) -> Result<SandboxConfig, ConfigError> {
    ...
    Ok(SandboxConfig {
        ...
        allowed_mounts: optional_string_array(sandbox, "allowedMounts", "merged settings.sandbox")?
            .unwrap_or_default(),
        default_timeout_ms: optional_u64(sandbox, "defaultTimeoutMs", "merged settings.sandbox")?,
        memory_limit_bytes: optional_u64(sandbox, "memoryLimitBytes", "merged settings.sandbox")?,
        output_limit_bytes: optional_usize(sandbox, "outputLimitBytes", "merged settings.sandbox")?,
    })
}
...
fn parse_filesystem_mode_label(value: &str) -> Result<FilesystemIsolationMode, ConfigError> {
    match value {
        ...
        "overlayfs" | "overlay-fs" => Ok(FilesystemIsolationMode::OverlayFs),
        other => Err(ConfigError::Parse(format!(
...
mod tests {
    ...
    fn parses_sandbox_config() {
        ...
        assert_eq!(loaded.sandbox().default_timeout_ms, Some(45_000));
        assert_eq!(loaded.sandbox().memory_limit_bytes, Some(134_217_728));
        assert_eq!(loaded.sandbox().output_limit_bytes, Some(8_192));

        fs::remove_dir_all(root).expect("cleanup temp dir");
    }

    #[test]
    fn parses_overlayfs_sandbox_mode() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(cwd.join(".claw")).expect("project config dir");
        ...
        fs::write(
            cwd.join(".claw").join("settings.local.json"),
            r#"{"sandbox": {"filesystemMode": "overlayfs"}}"#,
        )
        .expect("write local settings");
        ...
        assert_eq!(
            loaded.sandbox().filesystem_mode,
            Some(FilesystemIsolationMode::OverlayFs)
        );
    ...
    #[test]
    fn parses_permission_limits_leases_and_templates() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(cwd.join(".claw")).expect("project config dir");
        ...
        .expect("write permission settings");

        let loaded = ConfigLoader::new(&cwd, &home)
            .load()
            .expect("config should load");
        let permissions = loaded.permission_rules();

        assert_eq!(permissions.max_mode(), Some("danger-full-access"));
        assert_eq!(
            permissions.role_max().get("operator").map(String::as_str),
            Some("workspace-write")
        );
        assert_eq!(permissions.default_lease_seconds(), Some(90));
        assert_eq!(permissions.session_timeout_seconds(), Some(600));
        let template = permissions
            .templates()
            .get("tmp-cleanup")
            .expect("template should parse");
        assert_eq!(template.max_mode(), Some("workspace-write"));
        assert_eq!(template.allow(), &["bash(rm -rf tmp:*)".to_string()]);
        assert_eq!(template.ask(), &["bash(systemctl:*)".to_string()]);
