...
impl McpToolRegistry {
    ...
    pub fn register_server(
        ...
        self.register_server_with_prompts(
            server_name,
            status,
            tools,
            resources,
    ...
    pub fn register_server_with_prompts(
        &self,
        server_name: &str,
        status: McpConnectionStatus,
        tools: Vec<McpToolInfo>,
    ...
    pub fn get_prompt(
        ...
        let join_handle = std::thread::Builder::new()
            .name(format!("mcp-prompt-get-{server_name}-{prompt_name}"))
            .spawn(move || {
                ...
                runtime.block_on(async move {
                    ...
                    serde_json::to_value(response)
                        .map_err(|error| format!("failed to serialize MCP prompt result: {error}"))
                })
            })
            .map_err(|error| format!("failed to spawn MCP prompt call thread: {error}"))?;

        let value = join_handle.join().map_err(|panic_payload| {
            if let Some(message) = panic_payload.downcast_ref::<&str>() {
                format!("MCP prompt call thread panicked: {message}")
            } else if let Some(message) = panic_payload.downcast_ref::<String>() {
                format!("MCP prompt call thread panicked: {message}")
            } else {
                "MCP prompt call thread panicked".to_string()
            }
        })??;
...
mod tests {
    ...
    fn lists_prompts_from_connected_server() {
        ...
        registry.register_server_with_prompts(
            ...
            vec![McpPromptInfo {
                name: "triage".into(),
                description: Some("Triage prompt".into()),
                arguments: vec![McpPromptArgumentInfo {
                    name: "service".into(),
                    description: None,
                    required: true,
                }],
        ...
        let prompts = registry.list_prompts("srv").expect("prompts should list");
        assert_eq!(prompts.len(), 1);
        assert_eq!(prompts[0].name, "triage");
        assert_eq!(prompts[0].arguments[0].name, "service");
    }
