...
/// Workspace path policy with explicit outside-workspace allow-list roots.
...
impl WorkspacePathPolicy {
    ...
    fn validate(&self, resolved: &Path) -> io::Result<()> {
        let canonical_workspace = canonicalize_workspace_root(&self.workspace_root);
        if path_starts_with_normalized(resolved, &canonical_workspace) {
            return Ok(());
        }
        for allowed_root in &self.allowed_roots {
            let canonical_allowed = canonicalize_workspace_root(allowed_root);
            if path_starts_with_normalized(resolved, &canonical_allowed) {
                return Ok(());
            }
        }
        Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            format!(
                "path {} escapes workspace boundary {} and is not allow-listed",
                resolved.display(),
...
/// Stages workspace writes in a temporary layer until the caller commits them.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WorkspaceWriteLayer {
...
pub enum WorkspaceMergeConfirmation {
    Confirmed,
    Rejected,
}
...
pub struct WorkspaceMergeAuditEvent {
    pub action: String,
    pub target_path: String,
    pub layer_root: String,
    pub confirmed: bool,
    pub outcome: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct WorkspaceLayerCommit {
    pub output: WriteFileOutput,
    pub audit: WorkspaceMergeAuditEvent,
}

impl WorkspaceWriteLayer {
    pub fn new(
        workspace_root: impl Into<PathBuf>,
        layer_root: impl Into<PathBuf>,
    ) -> io::Result<Self> {
        let workspace_root = workspace_root.into();
        let layer_root = layer_root.into();
        let canonical_workspace = canonicalize_workspace_root(&workspace_root);
        let canonical_layer = canonicalize_workspace_root(&layer_root);
        if path_starts_with_normalized(&canonical_workspace, &canonical_layer)
            || path_starts_with_normalized(&canonical_layer, &canonical_workspace)
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "write layer root must be outside the workspace and must not contain it",
            ));
        }
        fs::create_dir_all(&layer_root)?;
        fs::write(layer_root.join(".claw-write-layer"), b"managed")?;
        Ok(Self {
            policy: WorkspacePathPolicy::new(workspace_root),
            layer_root,
        })
    }

    #[must_use]
    pub fn layer_root(&self) -> &Path {
        &self.layer_root
    }

    pub fn stage_write(&self, path: &str, content: &str) -> io::Result<WriteFileOutput> {
        let target_path = normalize_path_allow_missing(path)?;
        self.policy.validate(&target_path)?;
        let staged_path = self.staged_path_for_target(&target_path)?;
        write_file(staged_path.to_string_lossy().as_ref(), content)
    }

    pub fn commit_path(&self, path: &str) -> io::Result<WriteFileOutput> {
        self.commit_path_with_confirmation(path, WorkspaceMergeConfirmation::Confirmed)
            .map(|commit| commit.output)
    }

    pub fn commit_path_with_confirmation(
        &self,
        path: &str,
        confirmation: WorkspaceMergeConfirmation,
    ) -> io::Result<WorkspaceLayerCommit> {
        let target_path = normalize_path_allow_missing(path)?;
        self.policy.validate(&target_path)?;
        let staged_path = self.staged_path_for_target(&target_path)?;
        let audit_base = || WorkspaceMergeAuditEvent {
            action: "overlay-merge".to_string(),
            target_path: target_path.to_string_lossy().into_owned(),
            layer_root: self.layer_root.to_string_lossy().into_owned(),
            confirmed: confirmation == WorkspaceMergeConfirmation::Confirmed,
            outcome: "rejected".to_string(),
        };
        if confirmation != WorkspaceMergeConfirmation::Confirmed {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                serde_json::to_string(&audit_base()).unwrap_or_else(|_| {
                    "overlay merge rejected by missing confirmation".to_string()
                }),
            ));
        }
        let content = fs::read_to_string(&staged_path)?;
        let output = write_file(target_path.to_string_lossy().as_ref(), &content)?;
        Ok(WorkspaceLayerCommit {
            output,
            audit: WorkspaceMergeAuditEvent {
    ...
    pub fn cleanup(&self) -> io::Result<()> {
        let marker = self.layer_root.join(".claw-write-layer");
        if marker.exists() {
            fs::remove_dir_all(&self.layer_root)?;
        }
        Ok(())
    }

    fn staged_path_for_target(&self, target_path: &Path) -> io::Result<PathBuf> {
        let canonical_workspace = canonicalize_workspace_root(self.policy.workspace_root());
        let relative = target_path
            .strip_prefix(&canonical_workspace)
            .or_else(|_| target_path.strip_prefix(self.policy.workspace_root()))
            .map_err(|_| {
                io::Error::new(
                    io::ErrorKind::PermissionDenied,
                    format!(
                        "path {} cannot be staged outside workspace {}",
                        target_path.display(),
                        canonical_workspace.display()
                    ),
                )
            })?;
        Ok(self.layer_root.join(relative))
    }
}
...
fn glob_search_impl(
    ...
    let search_pattern = if pattern_is_absolute {
        pattern.replace('\\', "/")
    } else {
        pattern.replace('\\', "/")
    };
    ...
    for pat in &expanded {
        ...
        let walk_root = if pattern_is_absolute {
            derive_glob_walk_root(pat)
        } else {
            derive_relative_glob_walk_root(pat, &base_dir)
        };
        if let Some(root) = canonical_root.as_deref() {
        ...
        for entry in entries.flatten() {
            ...
            let candidate_display = if pattern_is_absolute {
                display_path_for_json(candidate)
            } else {
                candidate
                    .strip_prefix(&base_dir)
                    .map(display_path_for_json)
                    .unwrap_or_else(|_| display_path_for_json(candidate))
            };
            if entry.file_type().is_file()
                && compiled.matches(&candidate_display)
    ...
    let filenames = matches
        .into_iter()
        .take(100)
        .map(|path| display_path_for_json(&path))
        .collect::<Vec<_>>();
...
fn derive_relative_glob_walk_root(pattern: &str, base_dir: &Path) -> PathBuf {
    let path = Path::new(pattern);
    let mut prefix = PathBuf::new();
    for component in path.components() {
        let text = component.as_os_str().to_string_lossy();
...
/// Read a file with workspace boundary and explicit allow-list enforcement.
#[allow(dead_code)]
...
/// Write a file with workspace boundary and explicit allow-list enforcement.
#[allow(dead_code)]
...
/// Edit a file with workspace boundary and explicit allow-list enforcement.
#[allow(dead_code)]
...
mod tests {
    ...
    use super::{
        ...
        grep_search, is_symlink_escape, read_file, read_file_in_workspace, read_file_with_policy,
        write_file, write_file_with_policy, GrepSearchInput, WorkspaceMergeConfirmation,
        WorkspacePathPolicy, WorkspaceWriteLayer, MAX_WRITE_SIZE,
    };
    ...
    #[test]
    fn explicit_allowlist_permits_outside_workspace_path() {
        let workspace = temp_path("workspace-allowlist");
        let outside = temp_path("outside-allowlist");
        std::fs::create_dir_all(&workspace).expect("workspace dir should be created");
        std::fs::create_dir_all(&outside).expect("outside dir should be created");
        let outside_file = outside.join("allowed.txt");
        std::fs::write(&outside_file, "allow-listed").expect("outside file should write");

        let policy = WorkspacePathPolicy::new(&workspace).with_allowed_root(&outside);
        let read =
            read_file_with_policy(outside_file.to_string_lossy().as_ref(), None, None, &policy)
                .expect("allow-listed read should succeed");
        assert_eq!(read.file.content, "allow-listed");

        let write_target = outside.join("created.txt");
        write_file_with_policy(
            write_target.to_string_lossy().as_ref(),
            "created outside by explicit allow-list",
            &policy,
        )
        .expect("allow-listed write should succeed");
    ...
    #[test]
    fn policy_rejects_outside_path_without_allowlist() {
        let workspace = temp_path("workspace-no-allowlist");
        let outside = temp_path("outside-no-allowlist");
        std::fs::create_dir_all(&workspace).expect("workspace dir should be created");
        std::fs::create_dir_all(&outside).expect("outside dir should be created");
        let outside_file = outside.join("blocked.txt");
        std::fs::write(&outside_file, "blocked").expect("outside file should write");
        ...
        let error = result.unwrap_err();
        assert_eq!(error.kind(), std::io::ErrorKind::PermissionDenied);
        assert!(error.to_string().contains("not allow-listed"));
    ...
    #[test]
    fn workspace_write_layer_stages_then_commits_changes() {
        let workspace = temp_path("workspace-layer");
        let layer_root = temp_path("workspace-layer-stage");
        std::fs::create_dir_all(&workspace).expect("workspace dir should be created");
        let target = workspace.join("nested").join("draft.txt");
        let layer = WorkspaceWriteLayer::new(&workspace, &layer_root).expect("layer should create");

        layer
            .stage_write(target.to_string_lossy().as_ref(), "draft")
            .expect("stage write should succeed");

        assert!(!target.exists());
        assert_eq!(
            std::fs::read_to_string(layer_root.join("nested").join("draft.txt"))
                .expect("staged file should read"),
            "draft"
        );

        layer
            .commit_path(target.to_string_lossy().as_ref())
            .expect("commit should succeed");

        assert_eq!(
            std::fs::read_to_string(&target).expect("target file should read"),
            "draft"
        );
        layer.cleanup().expect("layer cleanup should succeed");
        assert!(!layer_root.exists());
        let _ = std::fs::remove_dir_all(&workspace);
    }

    #[test]
    fn workspace_write_layer_requires_explicit_merge_confirmation() {
        let workspace = temp_path("workspace-layer-confirm");
        let layer_root = temp_path("workspace-layer-confirm-stage");
        std::fs::create_dir_all(&workspace).expect("workspace dir should be created");
        let target = workspace.join("confirmed.txt");
        let layer = WorkspaceWriteLayer::new(&workspace, &layer_root).expect("layer should create");

        layer
            .stage_write(target.to_string_lossy().as_ref(), "draft")
            .expect("stage write should succeed");
        let rejected = layer.commit_path_with_confirmation(
            target.to_string_lossy().as_ref(),
            WorkspaceMergeConfirmation::Rejected,
        );

        assert!(rejected.is_err());
        assert!(!target.exists(), "rejected merge must not touch workspace");

        let committed = layer
            .commit_path_with_confirmation(
                target.to_string_lossy().as_ref(),
                WorkspaceMergeConfirmation::Confirmed,
            )
            .expect("confirmed merge should succeed");

        assert!(committed.audit.confirmed);
        assert_eq!(committed.audit.outcome, "merged");
        assert_eq!(
            std::fs::read_to_string(&target).expect("target file should read"),
            "draft"
        );
    ...
    #[test]
    fn workspace_write_layer_rejects_outside_workspace_targets() {
        let workspace = temp_path("workspace-layer-boundary");
        let layer_root = temp_path("workspace-layer-boundary-stage");
        let outside = temp_path("workspace-layer-outside.txt");
        std::fs::create_dir_all(&workspace).expect("workspace dir should be created");
        let layer = WorkspaceWriteLayer::new(&workspace, &layer_root).expect("layer should create");

        let result = layer.stage_write(outside.to_string_lossy().as_ref(), "outside");
