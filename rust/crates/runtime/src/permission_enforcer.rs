...
impl PermissionEnforcer {
    ...
    pub fn check_with_required_mode(
        ...
        let active_mode = self.policy.effective_active_mode();
        if self
            .policy
            .role_max_mode()
            .is_some_and(|max_mode| !max_mode.satisfies(required_mode))
        {
            let max_mode = self.policy.role_max_mode().expect("role max checked");
            return EnforcementResult::Denied {
                tool: tool_name.to_owned(),
                active_mode: active_mode.as_str().to_owned(),
                required_mode: required_mode.as_str().to_owned(),
                reason: format!(
                    "'{tool_name}' requires '{}' permission, exceeding role maximum '{}'",
        ...
        if active_mode.satisfies(required_mode) {
            return EnforcementResult::Allowed;
        }
    ...
    pub fn active_mode(&self) -> PermissionMode {
        self.policy.effective_active_mode()
    }
...
fn is_within_workspace(path: &str, workspace_root: &str) -> bool {
    let root = canonicalize_allow_missing(Path::new(workspace_root));
    ...
    path_starts_with_normalized(&resolved, &root)
}
...
mod tests {
    ...
    #[test]
    fn role_maximum_denies_dynamic_permission_escalation() {
        ...
        let result = enforcer.check_with_required_mode(
            "bash",
            r#"{"command":"systemctl restart sshd"}"#,
        ...
        match result {
            EnforcementResult::Denied {
                ...
                assert_eq!(active_mode, "workspace-write");
                assert_eq!(required_mode, "danger-full-access");
                assert!(reason.contains("role maximum"));
            }
            other => panic!("expected role maximum denial, got {other:?}"),
        }
    }
