...
const PERMISSION_TEMPLATE_FIELDS: &[FieldSpec] = &[
    FieldSpec {
        name: "allow",
        expected: FieldType::StringArray,
    },
...
pub fn validate_config_file(
    ...
    if let Some(permissions) = object.get("permissions").and_then(JsonValue::as_object) {
        ...
        if let Some(templates) = permissions.get("templates").and_then(JsonValue::as_object) {
            for (name, template) in templates {
                if let Some(template) = template.as_object() {
                    result.merge(validate_object_keys(
                        template,
                        PERMISSION_TEMPLATE_FIELDS,
                        &format!("permissions.templates.{name}"),
                        source,
                        &path_display,
                    ));
                }
