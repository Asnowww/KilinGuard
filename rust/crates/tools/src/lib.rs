...
use runtime::{
    ...
    write_file_with_policy, ApiClient, ApiRequest, AssistantEvent, BashCommandInput,
...
fn global_os_sense_runtime() -> Result<std::sync::Arc<std::sync::Mutex<OsSenseRuntime>>, String> {
...
pub fn start_os_sense_scheduler() -> Result<(), String> {
    use std::sync::OnceLock;
    static STARTED: OnceLock<Result<(), String>> = OnceLock::new();
    STARTED
        .get_or_init(|| {
            let runtime = global_os_sense_runtime()?;
            std::thread::Builder::new()
                .name("os-sense-collector".to_string())
                .spawn(move || loop {
                    ...
                    let next_due_ms = {
                        ...
                        if let Err(error) = runtime.tick(now) {
                            eprintln!("[os-sense] scheduled collection failed: {error}");
                        }
                        runtime.next_due_ms()
                    };
                    let sleep_ms = next_due_ms.saturating_sub(now).clamp(250, 60_000);
                    std::thread::sleep(Duration::from_millis(sleep_ms));
                })
                .map(|_| ())
                .map_err(|error| format!("failed to start OS sensing scheduler: {error}"))
        })
        .clone()
}

fn with_os_sense_runtime<T>(
    operation: impl FnOnce(&mut OsSenseRuntime) -> Result<T, String>,
) -> Result<T, String> {
    let runtime = global_os_sense_runtime()?;
    let mut runtime = runtime
        .lock()
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    let specs = vec![
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "filesystemMode": { "type": "string", "enum": ["off", "workspace-only", "allow-list", "overlayfs"] },
        ...
        ToolSpec {
            name: "OpsWorkflowRun",
            description: "Run or resume an operations workflow with durable checkpoint output.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "pluginName": { "type": "string", "pattern": "^[a-z][a-z0-9_-]{0,63}$" },
                    "toolName": { "type": "string", "pattern": "^[a-z][a-z0-9_-]{0,63}$" },
                    "requiredPermission": { "type": "string", "enum": ["read-only", "workspace-write", "danger-full-access"] },
                    "destination": { "type": "string", "maxLength": 512 }
                },
                "required": ["language", "pluginName", "toolName", "requiredPermission", "destination"],
                "additionalProperties": false
            }),
            required_permission: PermissionMode::WorkspaceWrite,
        },
        ...
        ToolSpec {
            name: "os_metrics_snapshot",
            description: "Collect and persist an on-demand read-only snapshot of OS CPU, memory, load, disk, platform, LoongArch hwmon sensor values, and threshold alerts.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "thresholds": {
        ...
        ToolSpec {
            name: "os_metrics_history",
            description: "Query persisted OS metric samples using one of the fixed time-series windows: 1h, 24h, or 7d.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            name: "os_sense_tick",
            description: "Run the configured periodic OS metric collector when its scheduler is due and persist the resulting sample.",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
        ...
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "process_allowed_names": { "type": "array", "maxItems": 200, "items": { "type": "string", "maxLength": 128 } },
                    "log_limit": { "type": "integer", "minimum": 1, "maximum": 500 }
                },
                "additionalProperties": false
            }),
            required_permission: PermissionMode::ReadOnly,
    ...
    #[cfg(not(windows))]
    {
        specs
            .into_iter()
            .filter(|spec| spec.name != "PowerShell")
            .collect()
    }
}
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        "OpsWorkflowRun" => {
            if enforcer.is_none() {
                return Err(
                    "OpsWorkflowRun requires a PermissionEnforcer so every workflow step is checked"
                        .to_string(),
                );
            }
            maybe_enforce_permission_check_with_mode(
            ...
            from_value::<OpsWorkflowRunInput>(input)
                .and_then(|input| run_ops_workflow(enforcer.cloned(), input))
        }
        "OpsPluginScaffold" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
            ...
            from_value::<OpsPluginScaffoldInput>(input).and_then(run_ops_plugin_scaffold)
        }
        "read_file" => {
        ...
        "os_metrics_snapshot" => {
            ...
            from_value::<OsMetricsSnapshotInput>(input).and_then(run_os_metrics_snapshot)
        }
        "os_metrics_history" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
            ...
            from_value::<OsMetricsHistoryInput>(input).and_then(run_os_metrics_history)
        }
        "os_sense_tick" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
            ...
            from_value::<EmptyInput>(input).and_then(run_os_sense_tick)
        }
        "os_process_list" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
            ...
            from_value::<OsProcessQuery>(input).and_then(run_os_process_list)
        }
        "os_log_query" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
            ...
            from_value::<OsLogQuery>(input).and_then(run_os_log_query)
        }
        "os_network_connections" => {
            ...
            from_value::<OsNetworkQuery>(input).and_then(run_os_network_connections)
        }
        "os_service_status" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
            ...
            from_value::<OsServiceQuery>(input).and_then(run_os_service_status)
        }
        "os_context_summary" => {
            maybe_enforce_permission_check_with_mode(
                enforcer,
...
#[allow(clippy::needless_pass_by_value)]
fn run_os_metrics_snapshot(input: OsMetricsSnapshotInput) -> Result<String, String> {
    with_os_sense_runtime(|runtime| {
        let snapshot = runtime
            .collect_metrics_on_demand(input.thresholds.as_ref())
            .map_err(|error| error.to_string())?;
        to_pretty_json(snapshot)
    })
}
...
#[allow(clippy::needless_pass_by_value)]
fn run_os_metrics_history(input: OsMetricsHistoryInput) -> Result<String, String> {
    with_os_sense_runtime(|runtime| {
        let history = runtime
            .query_history(input.window, current_time_ms())
            .map_err(|error| error.to_string())?;
        to_pretty_json(history)
    })
}

#[allow(clippy::needless_pass_by_value)]
fn run_os_sense_tick(_input: EmptyInput) -> Result<String, String> {
    with_os_sense_runtime(|runtime| {
        let now = current_time_ms();
        let snapshot = runtime.tick(now).map_err(|error| error.to_string())?;
        to_pretty_json(json!({
            "mode": "scheduled",
            "collected": snapshot.is_some(),
            "sample": snapshot,
            "next_due_ms": runtime.next_due_ms()
        }))
    })
}
...
fn run_ops_workflow(
    ...
    for tool_name in
        input
            ...
            .chain(
                input.workflow.steps.iter().filter_map(|step| {
                    step.rollback.as_ref().map(|rollback| rollback.tool.clone())
                }),
            )
            .collect::<BTreeSet<_>>()
    {
        if tool_name == "OpsWorkflowRun" {
            continue;
        }
        let registered_name = tool_name.clone();
        let step_enforcer = enforcer.clone();
        runner.register_tool(tool_name, move |value| {
            execute_tool_with_enforcer(step_enforcer.as_ref(), &registered_name, &value).and_then(
                |output| serde_json::from_str::<Value>(&output).map_err(|error| error.to_string()),
            )
        });
    }

    let mut result = if input.resume {
        runner.resume(&input.workflow, checkpoint)
    } else {
...
fn run_ops_plugin_scaffold(input: OpsPluginScaffoldInput) -> Result<String, String> {
    validate_scaffold_identifier(&input.plugin_name, "pluginName")?;
    validate_scaffold_identifier(&input.tool_name, "toolName")?;
    if input.destination.is_absolute()
        || input.destination.components().any(|component| {
            matches!(
                component,
...
fn validate_scaffold_identifier(value: &str, field: &str) -> Result<(), String> {
    ...
    if valid {
        ...
        Err(format!("{field} must match ^[a-z][a-z0-9_-]{{0,63}}$"))
    }
}

fn resolve_ops_checkpoint_path(path: &Path) -> Result<PathBuf, String> {
    if path
        .components()
        .any(|component| matches!(component, std::path::Component::ParentDir))
    {
        return Err("checkpointPath must not contain parent directory traversal".to_string());
    }

    let workspace = std::env::current_dir().map_err(|error| error.to_string())?;
    let allowed_root = workspace.join(".claw").join("workflow-checkpoints");
...
fn run_bash(input: BashCommandInput) -> Result<String, String> {
    ...
    sanitize_shell_output(&mut output);
    serde_json::to_string_pretty(&output).map_err(|error| error.to_string())
}

fn sanitize_shell_output(output: &mut BashCommandOutput) {
    output.stdout = strip_msys_startup_noise(&output.stdout);
    output.stderr = strip_msys_startup_noise(&output.stderr);
}

fn strip_msys_startup_noise(text: &str) -> String {
    text.lines()
        .filter(|line| !is_msys_startup_noise(line.trim()))
        .collect::<Vec<_>>()
        .join("\n")
}

fn is_msys_startup_noise(line: &str) -> bool {
    line.contains(r"C:\WINDOWS\system32\drivers\etc\hosts")
        && line.contains(" -> ")
...
fn run_read_file(input: ReadFileInput) -> Result<String, String> {
    let policy = workspace_path_policy()?;
    to_pretty_json(
        read_file_with_policy(&input.path, input.offset, input.limit, &policy)
...
fn run_write_file(input: WriteFileInput) -> Result<String, String> {
    let policy = workspace_path_policy()?;
    to_pretty_json(
        write_file_with_policy(&input.path, &input.content, &policy).map_err(io_to_string)?,
    )
}
...
fn workspace_path_policy() -> Result<WorkspacePathPolicy, String> {
    let workspace = std::env::current_dir().map_err(|error| error.to_string())?;
    let mut policy = WorkspacePathPolicy::new(&workspace);
    if let Ok(config) = ConfigLoader::default_for(&workspace).load() {
        for allowed in &config.sandbox().allowed_mounts {
            let allowed_root = if Path::new(allowed).is_absolute() {
                PathBuf::from(allowed)
            } else {
                workspace.join(allowed)
            };
            policy = policy.with_allowed_root(allowed_root);
        }
    }
    Ok(policy)
}
...
fn path_within_current_workspace(path: &str, allow_missing: bool) -> bool {
    ...
    if looks_like_windows_absolute_path(trimmed) && !cfg!(windows) {
        return false;
    }
    ...
    path_starts_with_normalized(&resolved, &cwd)
}
...
fn is_within_workspace(path: &str) -> bool {
    ...
    if path.is_absolute() {
        if let Ok(cwd) = std::env::current_dir() {
            let cwd = cwd.canonicalize().unwrap_or(cwd);
            let resolved = path.canonicalize().unwrap_or(path);
            return path_starts_with_normalized(&resolved, &cwd);
        }
    }
...
struct OsMetricsSnapshotInput {
    #[serde(default)]
    thresholds: Option<OsMetricsThresholds>,
}
...
fn execute_skill(input: SkillInput) -> Result<SkillOutput, String> {
    ...
    Ok(SkillOutput {
        skill: input.skill,
        path: display_path_for_json(&skill_path),
        args: input.args,
        description,
        prompt,
    })
...
fn display_path_for_json(path: &Path) -> String {
...
fn resolve_repl_runtime(language: &str) -> Result<ReplRuntime, String> {
    match language.trim().to_ascii_lowercase().as_str() {
        "python" | "py" => Ok(ReplRuntime {
            program: detect_first_command(python_runtime_candidates())
...
fn detect_powershell_shell() -> std::io::Result<String> {
    if let Some(path) = find_command_path("pwsh") {
        Ok(path)
    } else if let Some(path) = find_command_path("powershell") {
        Ok(path)
    } else {
...
fn python_runtime_candidates() -> &'static [&'static str] {
    if cfg!(windows) {
        &["python", "py", "python3"]
    } else {
        &["python3", "python"]
...
fn execute_shell_command(
    ...
    if run_in_background.unwrap_or(false) {
        let mut process = powershell_process(shell);
        process
            .arg("-NoProfile")
            .arg("-NonInteractive")
            .arg("-Command")
    ...
    if let Some(timeout_ms) = timeout {
        ...
        loop {
            if let Some(status) = child.try_wait()? {
                ...
                let stdout = strip_msys_startup_noise(&String::from_utf8_lossy(&output.stdout));
                let stderr = strip_msys_startup_noise(&String::from_utf8_lossy(&output.stderr));
                return Ok(runtime::BashCommandOutput {
                    stdout,
                    stderr,
                    raw_output_path: None,
...
fn powershell_process(shell: &str) -> std::process::Command {
    let shell_path = Path::new(shell);
    let extension = shell_path
        .extension()
  ...
  "nbformat_minor": 5
    ...
    fn glob_and_grep_tools_cover_success_and_errors() {
        ...
        assert!(path_string_ends_with(
            globbed_output["filenames"][0].as_str().expect("filename"),
            "nested/lib.rs"
        ));
...
"#
    ...
    #[test]
    fn os_sense_tool_specs_are_read_only() {
    ...
    #[test]
    fn os_sense_on_demand_collection_is_visible_in_fixed_history_window() {
        let snapshot =
            execute_tool("os_metrics_snapshot", &json!({})).expect("on-demand metrics collection");
        let snapshot: Value = serde_json::from_str(&snapshot).expect("snapshot json");
        assert!(snapshot["meta"]["collected_at_ms"].is_number());

        let history =
            execute_tool("os_metrics_history", &json!({"window": "1h"})).expect("history query");
        let history: Value = serde_json::from_str(&history).expect("history json");
        assert_eq!(history["window"], "1h");
    ...
    fn ops_workflow_tool_persists_checkpoint_and_runs_pipeline() {
        ...
        let checkpoint = PathBuf::from("incident.json");
        let workflow = json!({
            "name": "ops-entry",
            "steps": [
                {
        ...
        assert_eq!(
            value["outputs"]["second"]["structured_output"]["copied"],
            41
        );
        assert!(workspace
            .join(".claw")
    ...
    fn ops_plugin_scaffold_writes_linux_entrypoint_inside_workspace() {
        ...
        let output = registry
            .execute(
                ...
                &json!({
                    "language": "rust",
                    "pluginName": "kylin_ops",
                    "toolName": "inspect_host",
                    "requiredPermission": "read-only",
                    "destination": "plugins/kylin_ops"
                }),
            )
            .expect("scaffold");
        let value: Value = serde_json::from_str(&output).expect("json");
    ...
    #[test]
    fn ops_workflow_requires_enforcer_for_nested_steps() {
        let error = execute_tool(
            "OpsWorkflowRun",
            &json!({
                "workflow": {
                    "name": "deny-unscoped",
                    "steps": [
                        {"id": "danger", "tool": "bash", "input": {"command": "echo bypass"}}
                    ]
                },
                "checkpointPath": "deny-unscoped.json"
            }),
        )
        .expect_err("workflow without enforcer should be rejected");
        assert!(error.contains("requires a PermissionEnforcer"));
    }
    ...
    fn ops_workflow_executes_and_records_partial_rollback_on_failure() {
        ...
        let output = registry
            ...
            .expect("workflow returns structured failed result");
        let value: Value = serde_json::from_str(&output).expect("json");
        assert_eq!(value["status"], "failed");
        assert_eq!(value["rollbackResults"][0]["id"], "undo_prepare");
        assert_eq!(value["rollbackResults"][0]["succeeded"], true);
        assert_eq!(
    ...
    #[test]
    fn os_context_summary_runs_under_read_only_enforcer() {
    ...
    #[test]
    fn given_read_only_enforcer_when_plugin_tool_requires_write_then_denied_before_spawn() {
        ...
        let plugin_tool = PluginTool::new(
            ...
            PluginToolDefinition {
                name: "plugin_writer".to_string(),
                description: Some("write plugin".to_string()),
        ...
        let registry = GlobalToolRegistry::with_plugin_tools(vec![plugin_tool])
